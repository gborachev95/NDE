...
float4 main(INPUT_PIXEL _inputPixel) : SV_TARGET
{
	...
	if (point_light[lightIndex].color.a > 0)
	{
		...
		pointAttenuation = saturate(1.0f - (dot(pointLightDir, pointLightDir) / (point_light[lightIndex].radius * 2)));
		...
		// ***Edge Fuzz*** //
		//float3 viewDir = float3(viewMatrix._31, viewMatrix._32, viewMatrix._33);
		//float3 viewNormal = normalize(viewDir);
		//float3 Hn = normalize(viewNormal + pointLightDir);
		//float ldn = dot(pointLightDir.xyz, bumpNormal.xyz);
		//float4 lv = lit(ldn, dot(Hn, bumpNormal.xyz), 128);
		//float subd = abs(dot(bumpNormal.xyz, viewNormal));
		//subd = pow(subd, 32);
		//float3 EdgedColor = lerp(float3(1.0f,1.0f,1.0f), pointLightColor.xyz, subd);
		//EdgedColor = lerp(pointLightColor.xyz, EdgedColor, 2.0f);
		//ldn = max(ldn, 0);
